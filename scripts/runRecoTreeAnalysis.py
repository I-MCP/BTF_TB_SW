...
if (not file.IsOpen()):
    ...
    exit(-1)
tree = file.Get("eventData")

os.system('mkdir -p %s'%options.outputDir)

a=r.iMCP_BTF_recoTreeAnalysis(tree)
a.maxEntries=int(options.numberOfEvents)
a.outFile=options.outputDir+"/"+os.path.splitext(path_leaf(options.inputFile))[0]+"_recoTreeAnalysis.root"
a.Loop()
